-- Complete fix with corrected regex patterns
...
-- Step 3: Create the corrected function with proper regex syntax
...
BEGIN
    ...
    BEGIN
        ...
        IF response_data IS NOT NULL AND length(response_data) > 0 THEN
            -- Extract title using simpler regex pattern
            page_title := (regexp_match(response_data, '<title[^>]*>([^<]+)</title>', 'i'))[1];
            
            -- Extract description from meta tag - try multiple patterns
            page_description := (regexp_match(response_data, '<meta[^>]+name=["\']description["\'][^>]+content=["\']([^"'']+)["\']', 'i'))[1];
            
            -- Alternative description extraction if first didn't work
            IF page_description IS NULL THEN
                page_description := (regexp_match(response_data, '<meta[^>]+content=["\']([^"'']+)["\'][^>]+name=["\']description["\']', 'i'))[1];
            END IF;
